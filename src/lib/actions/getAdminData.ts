...
export interface IAdminData {
    entities: HydratedDocument<MergeType<IUser | IGroup, {msgs: IMsg[], purchases: MergeType<IPurchase, {giftGiven: IGift, msg: IMsg}>[]} | {msgs: IMsg[], purchases: MergeType<IPurchase, {giftGiven: IGift, msg: IMsg}>[], users: IUser[]}>>[];
}
...
export async function getAdminData({User, Group}: {User: Model<IUser>, Group: Model<IGroup>}): Promise<IAdminData | null> {
    ...
    const groups = await Group.find().populate<{msgs: IMsg[], users: IUser[]}>('msgs users').populate<{purchases: MergeType<IPurchase, {giftGiven: IGift, msg: IMsg}>[]}>({path: 'purchases', populate: {path: 'msg giftGiven'}});
