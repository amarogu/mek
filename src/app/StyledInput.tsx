                ...
                <div ref={divRef} className={`border-b relative h-96 pb-4 transition-colors ${res?.message === err ? 'border-red-400' : 'border-text-100 dark:border-dark-text-100' }`}>
                    <div className="flex flex-col gap-4">
                        ...
                        <div ref={descRef} className="relative origin-top-left">
                            {desc ? <p className={`text-xl ${res?.message === err ? 'opacity-0' : ''} transition-opacity text-text-100/75 dark:text-dark-text-100/75`}>{desc}</p> : null}
                            <p className={`absolute top-0 left-0 text-red-400/75 transition-opacity text-xl ${res?.message === err ? 'opacity-1' : 'opacity-0'}`}>{err}</p>
                        </div>
                    </div>
