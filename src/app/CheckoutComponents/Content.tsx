...
export default function Content({gift, item}: {gift: Gift, item?: User | Group}) {
    ...
    useGSAP(() => {
        switch (visualStep) {
            case 1:
                if (info.current) gsap.to(info.current, {opacity: 1});
                if (payment.current) gsap.to(payment.current, {opacity: 0, onComplete: () => setStep(1)});
                break;
            case 2:
                console.log('test');
                gsap.to(info.current, {opacity: 0, onComplete: () => setStep(2)});
                break;
        }
    }, [visualStep]);
    ...
    useGSAP(() => {
        switch (step) {
            case 1:
                gsap.to(info.current, {opacity: 1});
                break;
            case 2:
                gsap.to(payment.current, {opacity: 1});
                break;
        }
    }, [step]);
    ...
    const renderForm = () => {
        switch (step) {
            case 1:
                return (
                    <div ref={info} className="flex opacity-0 flex-col gap-12">
                        ...
                        <Button clicked={clicked} onComplete={() => setVisualStep(2)} cleanup={cleanup} onClick={handleClick} text="Próximo" res={res}  />
                    </div>
                )
            case 2:
                return (
                    <div ref={payment} className="flex opacity-0 flex-col gap-12">
                        <section className="flex flex-col gap-6">
                            <div className="flex gap-4 items-center">
                                <div className="p-4 h-[57px] bg-bg-200 dark:bg-dark-bg-200">
                                    <button onClick={goBack}><Image src={isDarkMode ? BackDark : Back} alt="Voltar" /></button>
                                </div>
