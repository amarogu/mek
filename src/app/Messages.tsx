...
import { ErrorResponse, Status, SuccessResponse } from "@/lib/helpers";
...
export default function Messages({ item, id }: { item?: User | Group, id: string }) {
    ...
    const handleClick = async (message: string) => {
        if (!message) {
            return () => {
                setStatus({status: 'failure', type: 'nonetwork'});
                setRes({message: 'Por favor, insira um recado para que você possa efetuar o envio.'} as ErrorResponse)
            }
        };
        setStatus({status: 'requested', type: 'nonetwork'});
        const res = await instance.post('/message', {owner: item?._id, content: message});
        if (res.status !== 200) return setRes(undefined);
        if (res.data.error) {
            return () => {
                setStatus({status: 'failure', type: 'network'});
                setRes(res.data as ErrorResponse)
            }
        }
        setStatus({status: 'success', type: 'network'});
        ...
        <section id={id} className="md:px-8">
            ...
            <form ref={formRef} className="flex px-8 md:px-0 lg:shrink lg:w-1/2 md:self-start flex-col gap-12">
                ...
                <div className="flex flex-col gap-12 lg:w-2/3">
                    ...
                    <Button status={status} onClick={() => handleClick(message)} res={res} text='Enviar' />
                </div>
            </form>
