    ...
    return (
        <div onClick={handleDivClick} className={`flex gap-9 border-text-200 cursor-pointer ${className} ${isTextBox ? 'h-full' : ''}`}>
            ...
            <div className="flex flex-col gap-3">
                <h3>{title}</h3>
                {isTextBox ? <textarea ref={inputRef as React.RefObject<HTMLTextAreaElement>} onChange={onChange} placeholder={description} className={`placeholder:text-xl resize-none placeholder:text-text-200 text-xl outline-none w-full bg-transparent ${isTextBox && 'h-full'}`} /> : <input ref={inputRef as React.RefObject<HTMLInputElement>} onChange={onChange} placeholder={description} className={`placeholder:text-xl placeholder:text-text-200 text-xl outline-none w-full bg-transparent`} />}
            </div>
        </div>
    )
}