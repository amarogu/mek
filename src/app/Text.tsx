...
export default function Text({on, el, content, animation}: TextProps) {
            ...
            switch (animation) {
                case 'upper-staggering':
                    useGSAP(() => {
                        ...
                        tl.from(chars.children, {y: -20, opacity: 0, stagger: 0.1});
        ...
        case 'mount':
            switch (animation) {
                case 'upper-staggering':
                    useGSAP(() => {
                        const chars = ref.current as HTMLElement;
                        gsap.from(chars.children, {y: -20, opacity: 0, stagger: 0.1});
                    }, []);
            }
            break;
        case 'click':
    ...
    const characters = content.split('').map((char, index) => <span key={index}>{char}</span>);
